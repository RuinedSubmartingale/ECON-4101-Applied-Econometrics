...
home.prices <- read.csv('./Data/hprice.csv', header = T)
```
...
```{r}
dispIncome.consump <- read.csv('./Data/cm04YC.csv', header=T)
...
# CM07
```{r}
data <- read.csv('./Data/tabb777.csv', header=T)
```
```{r}
str(data)
...
```{r}
dfm <- read.csv('./Data/burger78.csv')
...
dfm <- read.csv('./Data/astrohitters.csv')
summary(dfm)
```
```{r}
...
# 3/1/2017
```{r}
dfm <- read.csv('./Data/burger78.csv') # variables explained earlier above
```
...
# 3/6/2017
```{r}
dfm <- read.csv('./Data/goodyear.csv')
# price: $1000's
# size: sqft
# bedrooms: levels
# baths: levels
...
# }
dt <- as.data.table(read.csv('./Data/gewe.csv'))
...
# price (in takas)
# area (in acres)
df <- read.csv('./Data/sugarcane.csv')
y <- df$area
x <- df$price
```
...
# CM26 - 4/17/2017 - Univariate Time Series
## SEE CM26_UNIVARIATEFORECAST.R code!!!
```{r}
df <- fread('./Data/cm26.csv')
```

```{r}
str(df)
setorderv(df, c('year', 'quarter'))
n <- nrow(df)
invisible(df[, t := .I]) # indices 1 : nrow(df)
g.orig <- ggplot(df, aes(t, sales)) + geom_point() + geom_line(); 
g <- g.orig; g
```

### Components of Time Series
Given $X_t$ = time series data at time t  
The idea is that we can break this into several parts:  
$X_t = S_t + T_t + C_t$, where $S_t$ is a seasonal component that has regular/periodic variation to it, and $T_t$ is a trend component for gradual change, and $C_t$ is a cyclical component (e.g. business cycle), and $I_t$ is an irregular component (think of as an error). It's common practice to combine $T_t$ and $C_t$ into a single trend-cycle component $TC_t$. 

### Approaches to modeling
1. Additive Component Model
\[ X_t = TC_t + S_t + I_t \]
2. Multiplicative Component Model
\[ X_t = TC_t \cdot S_t \cdot I_t\]
3. Unobserved Components Model
\[ X_t = (TC_t + I_t) \cdot S_t \]

We're going to look at (3) Unobserved Components Model today.

#### Smoothing Data with Moving Average
e.g. Simple centered 3-point moving average
\[ x_t^* = \frac{x_{t-1} + x_{t} + x_{t+1}}{3} \]

#### Smoothing Quarterly Data with Moving Average
e.g. SMA(4): simple 4-point moving average
\[ x_{2.5}^* = \frac{x_1 + x_2 + x_3 + x_4}{4} \]

We want to center the above SMA(4). We do so by taking two-point simple moving average of the above points, i.e. a SMA(2) of the $x_t^*$.
\[ x_3^{**} = \frac{x_{2.5}^* + x_{3.5}^*}{2} \]
\[ \begin{aligned} 
x_t^{**} & = \frac{x_{t-.5}^* + x_{t+.5}^*}{2} \\ 
& = \frac{x_{t-2} + 2x_{t-1} + 2x_t + 2x_{t+1} + x_{t+2}}{5} \\ 
\end{aligned}\]

From the above, we create Seasonal Index: \[ SI_t = \frac{X_t}{X_t^{**}} \]

The next step is to calculate the median of our seasonal indices for each quarter: $ \text{median}(SI_{Q1}) \;,\; \text{median}(SI_{Q2}) \;,\;\text{median}(SI_{Q3}) \;,\;\text{median}(SI_{Q4})$
We should have the average of the medians of the four seasonal indices should equal to 1. If it's not equal to 1, then we scale them approriately to get the Adjusted Median Seasonal Indices. i.e. Suppose the average of the medians is equal to 1.25 instead of 1. Then we simply divide each median by 1.25 to scale them.

Our seasonally adjused data is then $X_t^s$ is then: 
\[ X_t^s = \frac{X_i}{AMSI_{Q_t}} \]
where AMSI is the Adjusted Median Seasonal Index, and $Q_t$ is the respective quarter (1,2,3 or 4) for time period $t$

```{r}
f <- rep(1/4, 4)
tc2.5 <- filter(df$sales, f, side=2); head(tc2.5)
f2 <- rep(1/2, 2)
df$tc3 <- filter(tc2.5, f2, side=1)
g <- ggplot(df, aes(t, sales)) + geom_point() + geom_line() + geom_line(aes(y=tc3), color = 'blue'); g
```

```{r}
# Create the seasonal indices
invisible(df[, si := sales / tc3])
str(df)
g <- ggplot(df, aes(t, si)) + geom_point() + geom_line(); g

# Split quarterly data
sq_medians <- sapply(split(df$si, df$quarter), median, na.rm = T)

# constant adjustment factor for medians
cfac <- 1 / mean(sq_medians); cfac

# seasonally adjusted quarterly medians
q_sa_medians <- sq_medians * cfac; q_sa_medians

# Q1 adjusted median is approx 1.75, meaning Q1 data is approx 75% higher

# seasonally adusted x's 
invisible(df[, sales_sa := sales / q_sa_medians])
```

```{r}
# black = original, blue = moving average, red = seasonally adjusted
g <- ggplot(df, aes(x=t)) + 
  geom_line(aes(y=sales)) + 
  geom_line(aes(y=tc3), color='blue') +
  geom_line(aes(y=sales_sa), color = 'red'); g
```

Given seasonal adjusted sales for the quarters are all equal to 632, we make the following predictions for the real sales amounts for those quarters:
```{r}
632 * sq_medians
```
...
Naive aprpoach: $x_{t+1} = C x_t$ = a constant multiple of your prior value  
Slightly better: $x_{t+1} = \beta_0 x_t + \beta_1 x_{t-1} + \dots$

# CM27 4/19/17 - Exponential Smoothing
L_t = exponential level in time period t
\[ 
\begin{aligned}
L_t & = \sum_{i=0}^{\infty}{\alpha (1-\alpha)^i x_{t-i}} \\
\implies L_t - (1-\alpha)L_{t-1} & = \alpha x_t \\
\implies L_t & = \alpha x_t + (1-\alpha) L_{t-1}
\end{aligned}
\]

We set $L_1 = x_1$. If $x_t$ is erratic, we set $\alpha \in (0,1)$ to a relatively low value. If $x_t$ is relativly stable, we set $\alpha$ to a relatively high value.

The error at time $t$ is $e_t = x_t - \hat{x}_t$. 

### Holt's Linear Trend Algorithm
Attempts to account for trend in the data.
\[
\begin{align}
\text{(Trend)} \quad T_t & = \beta (L_t - L_{t-1}) + (1 - \beta) T_{t-1} \\
\implies L_t & = \alpha x_t + (1-\alpha)(L_{t-1} + T_{t-1}) \\
& 0 < \alpha, \beta < 1
\end{align}
\]

e.g. of Holt's Linear Trend Algorithm:  
start with L_2 = x_2 and T_2 = x_2 - x_1  
and alpha = 0.3 and beta = 0.1  
...
for (i in 1:n) {
  if (i == 1) {
    ...
    L[i] <- alpha*df$sales_sa[i] + (1-alpha)*L[i-1]
  }
}
...
beta <- 0.1
str(df)
ggplot(df, aes(t, sales_sa)) + geom_line() + geom_line(aes(y = L), color = 'red')
```

```{r}
# Convert X data into a time series
df$sales_sa_ts <- ts(df$sales_sa, start=c(df$year[1], 1), frequency = 4) # 4 for quarterly
```

### Holt-Winters Function
Can account for not only exponential smoothing, but also for trends (seasonality).

```{r}
# gamma represents seasonal component. FALSE -> non-seasonal model is fitted
mod.hw <- HoltWinters(df$sales_sa_ts, alpha=0.3, beta=0.1, gamma=F)
plot(mod.hw) # looks very similar to the exponential smoothing plot above
mod.hw$SSE
```

```{r}
# allow function to AUTO choose best beta value for us
mod.hw2 <- HoltWinters(df$sales_sa_ts, alpha=0.3, gamma=F) 
mod.hw2$beta # as compared to our hardcoded 0.1 above
plot(mod.hw2) # looks similar, but less variance
mod.hw2$SSE # smaller than mod.hw$SSE since we let it auto choose beta
```

```{r}
# allow function to AUTO choose best alpha and beta values for us
mod.hw3 <- HoltWinters(df$sales_sa_ts, gamma=F)
mod.hw3$alpha; mod.hw3$beta
plot(mod.hw3)
mod.hw3$SSE
```
...
```{r}
library(forecast)
fcast <- forecast(mod.hw3, 4)
plot(fcast)
```

```{r}
# allow HoltWinters to account for seasonality
mod.hw4 <- HoltWinters(df$sales_sa_ts)
plot(mod.hw4)
fcast <- forecast(mod.hw4, 4)
fcast
plot(fcast)
```
...
# 4/24/2017
```{r}
df26 <- fread('Data/cm26.csv')
x <- ts(df26$sales, start=c(min(df26$year),1), frequency=4); x
...
mod.ses <- ses(xsa, h=4)
fcast.ses <- forecast(mod.ses, 4)
plot(fcast.ses)

# can also find forecast predictions as follows:
xsa.2008 <- predict(mod.ses, n.ahead=4); xsa.2008
```

```{r}
x.2008.ses <- si * xsa.2008$mean; x.2008.ses
alpha.ses <- mod.ses$model$par['alpha'] # alpha = exponential smoothing constant 
alpha.ses
```

```{r}
# Holt's Method
mod.h <- HoltWinters(xsa, gamma=F) # gamma = seasonality component constant
xsa.2008 <- predict(mod.h, n.ahead = 4); xsa.2008
x.2008.h <- si * xsa.2008; x.2008.h
alpha.h <- mod.h$alpha; alpha.h
beta.h <- mod.h$beta; beta.h # beta = trend component constant
```

```{r}
# Holt Winter's Method
# since we let Holt Winter's account for seasonality, we use the original 
# full data x, instead of our manually seasonally adjusted xsa data
...
mod.arima <- auto.arima(x, stepwise = F); mod.arima
fcast.arima <- forecast(mod.arima, 4); fcast.arima
plot(fcast.arima)

# The title for the above plot corresponds to: 
# ARIMA(AR, #Diffs, MA)(AR seasonal, #Diffs seasonal)(MA seasonal)[periods]
```
...
mod.holt <- holt(x)
accuracies <- lapply(list('fcast.ses' = fcast.ses, 'fcast.holt' = mod.holt, 'fcast.hw' = fcast.hw, 'fcast.arima' = fcast.arima, 'fcast.ssarima' = fcast.ssarima), accuracy)
accuracies
```

# 4/26/2017
\[ \text{success odds} = \frac{P(\text{success})}{1 - P(\text{success})} \]
\[ \text{Likelihood Ratio of 1 over 2} = \frac{\text{success odds 1}}{\text{success odds 2}} \]
## Logistic Transformation
\[ logit(y) \equiv \ln(\frac{y}{1-y}) = \ln(\text{odds ratio of y}) \]
\[ 
\begin{aligned}
\theta & \equiv \exp(\beta_0 + \beta_1x) \\
y & = \frac{\theta}{1 + \theta} \\
logit(y) & = \frac{\frac{\theta}{1 + \theta}}{1 - \frac{\theta}{1 + \theta}} \\
& = \theta \\
& = \exp(\beta_0 + \beta_1x)
\end{aligned}
\]
```{r}
dfm <- fread('Data/honorsSAT.csv')
str(dfm)
attach(dfm)
```
...
```{r}
summary(lm(hon ~ female))
# interpretation: being a female increases prob of being honors by 10.676%
```

#### Generalized Linear Models - Binomial Logistic Regression
```{r}
mod1 <- glm(hon ~ 1, family = binomial(link = logit))
summary(mod1)

odds.ratio <- exp(coef(mod1)[1]); odds.ratio
p.honors <- odds.ratio / (1 + odds.ratio); odds.ratio
```

```{r}
# LOGIT with female variable, female=1, male=0
mod2 <- glm(hon ~ female, family = binomial(link = logit))
summary(mod2)

odds.ratios <- exp(coef(mod2)); odds.ratios
# interpretation: being female increases likelihood of honors by 81%

exp(confint(mod2))
```

```{r}
# LOGIT with females and math scores
mod3 <- glm(hon ~ female + math, family = binomial(link = logit))
summary(mod3)

exp(coef(mod3))
# interpretation: 1 point increase in math score increases likelihood by 17.85%
# and after accounting for math scores, females are 2.63 as likely to get honors
